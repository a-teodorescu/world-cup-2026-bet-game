...
function renderLeaderboard() {
  ...
  const restHtml = rest.length ? rest.map((r) => {
    ...
    return `<article class="leaderboard-card leaderboard-row-card" aria-label="Locul ${r.rank}: ${escapeHtml(r.name)}">
      ...
      <div class="leaderboard-user"><strong>${escapeHtml(r.name)}</strong>${adminEmail}<span class="leaderboard-row-mobile-points" aria-hidden="true">${r.total} puncte</span><span class="leaderboard-row-breakdown">${r.exact} scoruri exacte • ${r.winner} (doar) pronosticuri corecte</span></div>
