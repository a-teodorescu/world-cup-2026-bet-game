...
function buildEmail({ user, periodLabel, totalLabel, rankLabel, dailyPointsLabel, dailyPoints, dailyExact, dailyWinner, items, siteUrl }) {
...
async function supabasePatch(baseUrl, anonKey, table, id, payload) {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Supabase PATCH ${table} a eșuat: ${text.slice(0, 300)}`);
  }
}

async function runOneScheduledTest(test, env) {
  const { BREVO_API_KEY, BREVO_FROM_EMAIL, BREVO_FROM_NAME, SUPABASE_URL, SUPABASE_ANON_KEY, SITE_URL } = env;
  ...
  try {
    ...
    console.warn('[WC2026 one-time emails] match overrides not available', err.message);
  }
  ...
  const dailyPointsLabel = `Puncte la data ${formatRoDate(reportDate)}`;
  const subject = noResultsMode ? `Nu s-au jucat meciuri - ${formatRoDate(reportDate)}` : `Rezumat pronosticuri - ${formatRoDate(reportDate)}`;
  const summary = { attempted: 0, sent: 0, skippedDuplicate: 0, failed: 0, details: [], reportDate, matchCount: reportMatches.length, noResults: noResultsMode };
  ...
  for (const player of players) {
    ...
    const content = noResultsMode
      ...
      : buildEmail({ user: reportUser, periodLabel, totalLabel, rankLabel, dailyPointsLabel, dailyPoints, dailyExact, dailyWinner, items, siteUrl: SITE_URL });
    summary.attempted += 1;
    try {
      ...
      await supabaseInsert(SUPABASE_URL, SUPABASE_ANON_KEY, 'wc2026_email_logs', {
        ...
        payload: { testId: test.id, dailyPoints, dailyExact, dailyWinner, total: reportUser.total, rank: reportUser.rank, matchCount: items.length, noResults: noResultsMode }
      });
      summary.sent += 1;
...
exports.handler = async () => {
  ...
  const pending = await supabaseGet(
    SUPABASE_URL,
    SUPABASE_ANON_KEY,
    'wc2026_scheduled_email_tests',
    `?select=id,report_date,run_at,status&status=eq.pending&run_at=lte.${encodeURIComponent(nowIso)}&order=run_at.asc&limit=3`
  );
  if (!pending.length) return json(200, { ok: true, checkedAt: nowIso, pending: 0 });

  const results = [];
  for (const test of pending) {
    try {
      await supabasePatch(SUPABASE_URL, SUPABASE_ANON_KEY, 'wc2026_scheduled_email_tests', test.id, { status: 'running', started_at: new Date().toISOString() });
      const summary = await runOneScheduledTest(test, { BREVO_API_KEY, BREVO_FROM_EMAIL, BREVO_FROM_NAME, SUPABASE_URL, SUPABASE_ANON_KEY, SITE_URL });
      await supabasePatch(SUPABASE_URL, SUPABASE_ANON_KEY, 'wc2026_scheduled_email_tests', test.id, {
        status: summary.failed > 0 ? 'partial' : 'sent',
        finished_at: new Date().toISOString(),
        summary
      });
      results.push({ id: test.id, status: summary.failed > 0 ? 'partial' : 'sent', ...summary });
    } catch (error) {
      await supabasePatch(SUPABASE_URL, SUPABASE_ANON_KEY, 'wc2026_scheduled_email_tests', test.id, {
        status: 'error',
        finished_at: new Date().toISOString(),
      ...
      results.push({ id: test.id, status: 'error', error: error.message });
    }
  }
  return json(200, { ok: true, checkedAt: nowIso, processed: results.length, results });
};
