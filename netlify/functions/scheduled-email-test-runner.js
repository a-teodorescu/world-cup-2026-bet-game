...
async function runOneScheduledTest(test, env) {
  ...
  if (reportMatches.length === 0) {
    return { skipped: true, reason: 'Nu există rezultate salvate pentru data selectată.', reportDate, sent: 0, failed: 0, skippedDuplicate: 0 };
  }
  ...
  const summary = { attempted: 0, sent: 0, skippedDuplicate: 0, failed: 0, details: [], reportDate, matchCount: reportMatches.length };
  ...
  for (const player of players) {
    ...
    try {
      ...
      await supabaseInsert(SUPABASE_URL, SUPABASE_ANON_KEY, 'wc2026_email_logs', {
        ...
        payload: { testId: test.id, dailyPoints, dailyExact, dailyWinner, total: reportUser.total, rank: reportUser.rank, matchCount: items.length }
      });
      summary.sent += 1;
